...
var customize_id_token = false;

/**
 * Only allow confidential clients to introspect or revoke tokens?
 */
var only_allow_conf_client_introspect = false;
...
function produceClaim(claimName, expectedValues, isEssential, stateId, toSave) {
	...
	if (value == null) {
		value = stsuu.getAttributeContainer()
				.getAttributeValueByNameAndType(claimName, "urn:ibm:names:ITFIM:5.1:accessmanager");
		if (value != null) {
			toSave.push({"key" : ""+claimName, "value" : ""+value});
		}
	}
...
if (customize_id_token) {
	...
	if (request_type == "authorization") {
		...
		} else if ((response_type != null && response_type.indexOf("code") > -1 && is_oidc_scope)) {
